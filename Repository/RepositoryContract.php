...
class RepositoryContract implements RepositoryInterface{
    ...
    public function save(object $contract):void{
        ...
        $sql = "INSERT INTO Contrat(Personnes_id,Equipe_id,Salaire,Date_de_fin,Clause_de_rachat) VALUES(?,?,?,?,?)";
        $stmt = $this->pdo ->prepare($sql);
        $stmt->execute([$contract->getPersonnes_id(),$contract->getEquipe_id(),$contract->getSalaire(),$contract->getDate_de_fin(),$contract->getClause_de_rachat()]);
